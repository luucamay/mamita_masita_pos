-- Aggregated reporting views keep the report shape stable while allowing the
-- UI to switch between daily, weekly, and monthly sales periods.
...
group by date, product_or_service, unit_price, payment_method;
...
select
  ...
  payment_method
from public.v_sales_report_line_items
group by date_trunc('week', date::timestamp)::date, product_or_service, unit_price, payment_method;
...
select
  ...
  payment_method
from public.v_sales_report_line_items
group by date_trunc('month', date::timestamp)::date, product_or_service, unit_price, payment_method;
